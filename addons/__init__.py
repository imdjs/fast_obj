...
# else:
    # try:
        # exec("import "+G.文件夹此IOOBJ+".PYLIB_"+G.文件夹此IOOBJ+".global_var_LIB as LG");
    # except:
        # import PYLIB.global_var_LIB as LG;         
        
#------------------------------
# if("PYLIB.PYLIB_main" not in sys.modules):
...
class 卐卸载DLLobj卐Operator(bpy.types.Operator):
    ...
    def execute(self, context):
        if(G.DLLobj):
            ...
            except:print("!!! ERROR FreeLibrary DLLobj==",G.DLLobj);
...
class 导入OBJ(bpy.types.Operator, ImportHelper):
    ...
    bp检查文件破损 : BoolProperty(name="check file",description="Check the file for any damage(this will spend more time)",default=False,);
    bp用平滑组 : BoolProperty(name="Smooth Groups",description="Surround smooth groups by sharp edges",default=False,)

    #bpSelected = BoolProperty(name="Only Selected",description="Only Selected Objects",default=True, )
    bp分裂物 : BoolProperty(name="Split as Objects",description="Import OBJ Objects into Blender Objects",default=False, )   
